						...
						<div class="shop-other-info">
							...
							<div class="shop-add-score">
								...
								<div class="shop-score">
									...
									<span class="shop-score-arrow-span">
										<i class="shop-score-arrow"></i>
									</span>
									<div class="shop-score-box">
										<div>描述相符：
											<en-shop-star :star="shop.shop_description_credit"/>
										</div>
										<div>服务态度：
											<en-shop-star :star="shop.shop_service_credit"/>
										</div>
										<div>发货速度：
											<en-shop-star :star="shop.shop_delivery_credit"/>
										</div>
									</div>
								</div>
							</div>
							<div class="expanded-opt">
								<a href="javascript:;" class="expanded-opt-a" @click="shop.goods_on = !shop.goods_on">
					...
					<div :class="['shop-goods', shop.goods_on && 'on']">
						<template v-if="!shop.goods_list || !shop.goods_list.length">
							<p class="no-goods">哎呀！店铺还没商品呢，去
								<nuxt-link :to="'/shop/' + shop.shop_id">店铺首页</nuxt-link>看看试试吧！
							</p>
						</template>
						<ul v-else>
...
export default {
	...
	methods: {
		...
		/** 格式化地址信息 */
		formatAddress(shop) {
			return `${shop.shop_province} ${shop.shop_city} ${
				shop.shop_county
			} ${shop.shop_town}`;
		}
	}
