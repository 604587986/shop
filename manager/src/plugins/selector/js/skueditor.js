...
\
    ...
    .sku-container .sku-tr .td-section .sku-group-item >p{\
        display: none;\
    }\
    .sku-container .sku-tr .td-section .sku-group-item:first-child >p{\
        padding-left: 5px;\
    }\
            ...
            <div class='sku-tr'>\
                ...
                <div class='sku-item-model' style='display: none;'>\
                    ...
                    <div class='sku-group-item'>\
                        ...
                        <p class='spec-image-info'>目前只支持为第一个规格添加不同的规格图片！</p>\
                    </div>\
                    <!-- 规格值的模型 -->\
...
var specValueSelect = undefined; // 保存当前规格值选择框的selectize实例
...
var bindSelectizeJS = function(which) {
    $(which.find('.select-sku')).selectize({
        ...
        onItemAdd: function(value, $item) {
            ...
            axios
                .post(
                    '/goods-info/seller/category/' + category_id + '/spec.do',
                    {
                        cat_id: category_id,
                        spec_name: specName
                    }
    ...
    var $select = $(which.find('.select-sku-value')).selectize({
        ...
        onOptionAdd: function(value, data) {
            ...
            axios
                .post('/goods-info/seller/spec/' + specId + '/spec-values.do', {
                    spec_id: specId,
...
var computedSepcNameArray = function() {
    var selectedArray = [];
    skuDOM.find('select.select-sku option[selected=selected]').each(function() {
...
var createAndInsertSkuValue = function(where, valuesObj, specName, topItemDOM) {
    ...
    for (var prop in valuesObj) {
        if (prop && !specValueIsExist(where, specName, prop)) {
            ...
            //         // 把预览图片的DOM移动到上方
            //         $(element).prepend($(element).find('span.placeholdermap'));
...
var getSpecItemNameIdObject = function() {
    ...
    skuDOM
        ...
        .each(function() {
            nameIdObj[$(this).text()] = Number($(this).attr('data-value'));
        });
    return nameIdObj;
};
...
var adjustSpecValueDialog = function() {
    var addButton = skuDOM.find(' .td-section .sku-group-item .add-sku-value');
            ...
            .each(function(index) {
                ...
                tempSkuArray.push({
                    ...
                    spec_type: specValueObject.spec_type
                        ? Number(specValueObject.spec_type)
                        : initSpecValueObj
                          ? Number(initSpecValueObj.spec_type)
                          : 0,
                    spec_image: specValueObject.spec_image
                        ? specValueObject.spec_image
                        : initSpecValueObj ? initSpecValueObj.spec_image : ''
                });
            });
...
var initDOM = function() {
    ...
    // 删除当前规格项
    skuDOM
        .find('.sku-tr .sku-item-model .sku-group-item .bg-grey .delete-item')
        .click(function() {
    ...
    skuDOM.find('.sku-item-model .value-item .delete').click(function() {
        ...
        // 隐藏表格容器div
        if (skuDOM.find('.sku-table .tr-quantity-div tr').length === 0) {
            skuDOM.find('.sku-table .tr-quantity-div').css('display', 'none');
        } else {
            skuDOM.find('.sku-table .tr-quantity-div').css('display', 'flex');
        }
    });
    ...
    skuDOM.find('.sku-tr .sku-item-model .add-sku-value').click(function() {
        ...
        // 每次显示选择框前，清空已选择的item(如果上一次用户新增item的话，则该值会在下一次默认被选择)
        specValueSelect[0].selectize.clear();
        $(this)
    ...
    data.forEach(function(skuObj) {
        ...
        skuObj.specList.forEach(function(spec) {
            ...
            if (specDataObj.hasOwnProperty(spec.spec_name)) {
                if (
                    !specDataObj[spec.spec_name].hasOwnProperty(spec.spec_value)
                ) {
