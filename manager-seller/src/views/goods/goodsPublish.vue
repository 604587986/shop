<template>
	<div v-loading="loading">
		...
		<div class="content-goods-publish" v-show="activestep === 0">
			<div class="goods-category">
				<ul v-if="categoryListLevel1 && categoryListLevel1.length > 0">
					<li v-for="(item,index) in categoryListLevel1" :class="{'activeClass':index == activeCategoryIndex1}" @click="handleSelectCategory(item, index, 1)">
						<span>{{ item.name }}</span>
						<span>》</span>
					</li>
				</ul>
				<ul v-if="categoryListLevel2 && categoryListLevel2.length > 0">
					<li v-for="(item,index) in categoryListLevel2" :class="{'activeClass':index == activeCategoryIndex2}" @click="handleSelectCategory(item,index, 2)">
						<span>{{ item.name }}</span>
						<span>》</span>
					</li>
				</ul>
				<ul v-if="categoryListLevel3 && categoryListLevel3.length > 0">
					<li v-for="(item,index) in categoryListLevel3" :class="{'activeClass':index == activeCategoryIndex3}" @click="handleSelectCategory(item,index, 3)">
						<span>{{ item.name }}</span>
						<span>》</span>
					</li>
				</ul>
		...
		<div class="content-goods-publish" v-show="activestep === 1">
			<!--商品详情-->
			<el-form :model="baseInfoForm" status-icon :rules="baseInfoFormRule" label-position="right" ref="baseInfoForm" label-width="120px" class="demo-ruleForm">
				<!--商品详情-->
				<!--基本信息-->
				<div class="base-info-item">
					<h4>基本信息</h4>
					<div>
						<el-form-item class="auth-info" label="拒绝原因：" v-if="baseInfoForm.is_auth === 2">{{ baseInfoForm.auth_message }}</el-form-item>
						...
						<el-form-item label="店铺商品分组：">
							<!--商品分组 获取分类列表 传入默认值-->
							<en-category-picker @changed="changeGoodsCateGory" :clearable="true" :defaultVal="baseInfoForm.shop_cat_id"/>
						</el-form-item>
						<p class="goods-group-manager">商品可以从属于店铺的某个分组之下，店铺分组可以由 "商家中心 -> 商品管理 -> 分组管理" 中自定义</p>
						<el-form-item label="商品品牌：">
							<el-select v-model="baseInfoForm.brand_id" filterable @change="changeGoodsBrand" clearable placeholder="请选择">
					...
					<div>
						<el-form-item label="seo标题：">
							<el-input
								placeholder="最多60个字符"
								class="seo-text"
								@change="() => { baseInfoForm.page_title = baseInfoForm.page_title.trim() }"
								:maxlength="60"
								v-model="baseInfoForm.page_title"
							></el-input>
						</el-form-item>
				...
				<div class="base-info-item">
					...
					<div>
						<el-form-item label="运费：" style="width: 50%;" prop="template_id">
							...
							<el-select v-model="baseInfoForm.template_id" placeholder="请选择运费模板" v-show="baseInfoForm.goods_transfee_charge === 0 " @change="changeTpl">
								<el-option v-for="item in tplList" :key="item.id" :label="item.name" :value="item.id"></el-option>
							</el-select>
						</el-form-item>
					</div>
				</div>
				...
				<div class="base-info-item" v-if="isShowExchangeConfig">
					...
					<div v-if="baseInfoForm.exchange">
						...
						<el-form-item label="积分商品分类：">
							<el-select v-model="baseInfoForm.exchange.category_id" placeholder="请选择积分商品分类" @change="changeExangeCategory">
								<el-option v-for="item in exchangeGoodsCatrgoryList" :key="item.category_id" :label="item.name" :value="item.category_id"></el-option>
							</el-select>
						</el-form-item>
					</div>
				</div>
				...
				<el-collapse :value="collapseVal">
					...
					>
						...
						>
							<el-input placeholder="长度为最多100个字符" maxlength="100" v-if="goods_params_list.param_type === 1" v-model="goods_params_list.param_value"></el-input>
							<el-select v-if="goods_params_list.param_type === 2" v-model="goods_params_list.param_value" placeholder="请选择">
								<el-option v-for="(option) in goods_params_list.option_list" :key="option" :label="option" :value="option"></el-option>
							</el-select>
						</el-form-item>
					</el-collapse-item>
				</el-collapse>
		...
		<div class="content-goods-publish" v-if="activestep === 2">
			<el-form :model="baseInfoForm" label-position="right" ref="baseInfoFormIntro" label-width="120px" class="demo-ruleForm">
		...
		<div class="footer">
			<el-button-group>
				...
				<el-button type="primary" @click="aboveGoods" v-if="activestep === 1 || activestep === 2">上架</el-button>
				<el-button
					type="primary"
					@click="handleUnderGoods"
					v-if="currentStatus === 1 && ( activestep === 1 || activestep === 2) && baseInfoForm.market_enable === 1"
				>下架</el-button>
				<el-button type="primary" @click="saveDraft" v-if="(currentStatus != 1 && activestep != 0)">保存草稿</el-button>
			</el-button-group>
		</div>
	</div>
</template>
...
export default {
		...
		const checkMarket = (rule, value, callback) => {
			...
			setTimeout(() => {
				if (!RegExp.money.test(value)) {
					...
					if (
						parseFloat(value) < parseFloat(this.baseInfoForm.cost)
					) {
		...
		const checkCost = (rule, value, callback) => {
			...
			setTimeout(() => {
				if (!RegExp.money.test(value)) {
					...
					if (
						parseFloat(value) >
						parseFloat(this.baseInfoForm.mktprice)
					) {
						callback(new Error("市场价格必须大于等于成本价格"));
					} else {
						callback();
		...
		const checkExchangePoint = (rule, value, callback) => {
			...
			setTimeout(() => {
				if (
					...
					if (
						parseInt(this.baseInfoForm.exchange.exchange_point) <= 0
					) {
	...
	methods: {
		...
		GET_NextLevelCategory(row, level) {
			...
			API_goodsCategory.getGoodsCategoryLevelList(_id, {}).then(
				response => {
					this.loading = false;
					if (level === 1) {
		...
		GET_GoodData() {
			API_goods.getGoodData(this.activeGoodsId, {}).then(response => {
				...
				/** 查询商品sku信息 */
				API_goods.getGoodsStockList(this.activeGoodsId, {}).then(
					response => {
						/** 构造临时规格数据 */
				...
				API_goods.getGoodsParams(this.baseInfoForm.category_id).then(
					response => {
						...
						this.goodsParams.forEach(key => {
							if (key && key.params) {
								key.params.forEach(elem => {
									this.baseInfoForm.goods_params_list.push(
										elem
									);
								});
							}
		...
		/** 查询单个草稿箱商品信息 */
		GET_GoodDraftData() {
			API_goods.getGoodDraftData(this.activeGoodsId, {}).then(
				response => {
					/** 此处完成商品信息赋值 进行判断如果有值的话 */
					...
					/** 查询草稿箱sku信息 */
					API_goods.draftSku(this.activeGoodsId, {}).then(
						response => {
							/** 构造临时规格数据 */
							this.skuList = response.map(key => {
								if (
									...
									const spec_list = key.spec_list.map(
										item => {
											let {
												spec_id,
												spec_image,
		...
		setSort() {
			...
			this.sortable = Sortable.create(el, {
				...
				onEnd: evt => {
					let temp = this.baseInfoForm.goods_gallery_list[
						evt.oldIndex
					];
					this.baseInfoForm.goods_gallery_list[
						evt.oldIndex
			...
			this.baseInfoForm.sku_list.forEach(key => {
				...
				if (
					!(
						...
						parseInt(key.quantity) < 99999999
					) ||
