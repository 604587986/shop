...
export function saveShopTheme(id, params) {
  ...
  return new Promise((resolve, reject) => {
    request({
      url: `http://www.andste.cc/mock/5aa72c080d9d060b4b99b45b/seller/shop/themes/pc/${id}`,
      method: 'post',
      data: _formData
...
export function getShopWapThemeList(params) {
  return new Promise((resolve, reject) => {
    request({
      url: 'http://www.andste.cc/mock/5aa72c080d9d060b4b99b45b/seller/shop/themes/wap/list',
      method: 'get',
      loading: false,
      params
    }).then(response => {
 ...
 * @returns {Promise<any>}
 */
export function saveWapShopTheme(id, params) {
  const _formData = new FormData()
  Object.keys(params).forEach(key => _formData.append(key, params[key]))
  return new Promise((resolve, reject) => {
    request({
      url: `http://www.andste.cc/mock/5aa72c080d9d060b4b99b45b/seller/shop/themes/wap/${id}`,
      method: 'post',
      data: _formData
    }).then(response => resolve(response)).catch(error => reject(error))
  })
