<template>
  <div class="container">
    ...
    <el-dialog title="设置价格区间" :visible.sync="dialogVisible" width="450px">
      <el-form label-width="50px" class="price-range-form">
        <template v-for="(item, index) in defaultRange">
          <el-form-item :label="'第' + (index + 1) + '组'">
            <el-input-number size="mini" v-model="item[0]" disabled :controls="false"></el-input-number>
             -
            <el-input-number size="mini" v-model="item[1]" :disabled="index !== defaultRange.length - 1" :controls="false"></el-input-number>
            <template v-if="index === defaultRange.length - 1">
  ...
  export default {
    ...
    methods: {
      ...
      handleAddRange() {
        ...
        if (this.defaultRange.length >= this.maxRange || !this.validLastVal()) return
        // 取最后一个数组
      ...
      handleDeleteRange() {
        // 如果还剩一个，则不删除
        if (this.defaultRange.length <= 1) return
        this.defaultRange.pop()
      },
      // 校验最后一个数是否符合要求
      validLastVal() {
        const defaultRange = this.defaultRange
      ...
      handleConfirm() {
        ...
        this.$emit('changed', this.defaultRange)
