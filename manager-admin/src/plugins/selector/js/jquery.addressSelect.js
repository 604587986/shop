  ...
  addressSelect.prototype = {
    ...
    useApi: function (res, region_grade) {
      ...
      for (var i = 0, len = result.length; i < len; i++) {
        var local_title = result[i]['local_name'],
          local_name  = local_title.substring(0, 6);
    ...
    setNameTag: function (region_grade, region_id, local_name) {
      ...
      switch (parseInt(region_grade)) {
        ...
        case 2:
          ...
          _ele.find("input[name=" + names.city_id + "]").val(region_id).nextAll().val('');
        ...
        case 3:
          ...
          _ele.find("input[name=" + names.region_id + "]").val(region_id).nextAll().val('');
        ...
        case 4:
          ...
          _ele.find("input[name=" + names.town_id + "]").val(region_id).nextAll().val('');
    ...
    complyDeApi: function (res) {
      ...
      for (var i = 0, len = result.length; i < len; i++) {
        ...
        if (_this.defaultArray[_this.index + 1] && region_id === _this.defaultArray[_this.index + 1].re_id) {
      ...
      $.ajax({
        ...
        success: function (res) {
          _this.defaultArray.length > 0
            ...
            : _this.useApi(res, _region_grade);
        }
      });
    },
    ...
    initElement: function () {
      ...
      var province    = names.province ? '<input type="hidden" value="" name="' + names.province + '"/>' : '',
        province_id = names.province_id ? '<input type="hidden" value="" name="' + names.province_id + '"/>' : '',
        city        = names.city ? '<input type="hidden" value="" name="' + names.city + '"/>' : '',
        city_id     = names.city_id ? '<input type="hidden" value="" name="' + names.city_id + '"/>' : '',
        region      = names.region ? '<input type="hidden" value="" name="' + names.region + '"/>' : '',
        region_id   = names.region_id ? '<input type="hidden" value="" name="' + names.region_id + '"/>' : '',
        town        = names.town ? '<input type="hidden" value="" name="' + names.town + '">' : '',
        town_id     = names.town_id ? '<input type="hidden" value="" name="' + names.town_id + '">' : '';
    ...
    /* 初始化创建css样式
		...
		createStyles : function(){
		  if($('#addressSelectStyls').length !== 0) return;
			var style = document.createElement('style'),
				head  = document.head || document.getElementsByTagName('head')[0];
			style.type = 'text/css';
			style.id = 'addressSelectStyls';
			if(style.styleSheet){
				var func = function(){
