<template>
	<el-dialog :visible.sync="dialogVisible" :close-on-click-modal="false" :close-on-press-escape="false" append-to-body width="805px">
		<div slot="title" class="image-picker-title">文件上传</div>
	...
	methods: {
		...
		handleEditItem(file) {
			...
			if (deTarget && deTarget.opt) {
				curData.operation.forEach(
					opt => (opt.value = deTarget.opt[opt.name])
				);
			}
