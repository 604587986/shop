    ...
    methods: {
      ...
      handleDeleteSiteMenu(index, row) {
        this.$confirm('确定要删除这个导航菜单吗？', '提示', { type: 'warning' }).then(() => {
          API_SiteMenu.deletePCSiteMenu(row.id).then(response => {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if (this.siteMenuForm.form_type === 'add') {
              API_SiteMenu.addPCSiteMenu(this.siteMenuForm).then(response => {
                this.dialogSiteMenuVisible = false
                this.$message.success('添加成功！')
                this.GET_SiteMenuList()
              }).catch(error => console.log(error))
            } else {
              API_SiteMenu.editPCSiteMenu(this.siteMenuForm.id, this.siteMenuForm).then(response => {
                this.dialogSiteMenuVisible = false
      ...
      GET_SiteMenuList() {
        this.loading = true
        API_SiteMenu.getSiteMenuByPC(this.params).then(response => {
          this.loading = false
